...
def gaussian_kernel_cross_validation(dataset_name, dataset_train, dataset_test, approach, approach_name):
    ...
    plt.title(f'dataset: {dataset_name}, L_mejor= {L_mellor}, G_mejor: {G_mellor}, kappa={kappa_mellor:.2f}%')
    #plt.show()
    plt.savefig("sintonizacion_SVC_gaussian_"+approach_name+ "_" + dataset_name + ".png"); plt.clf()
    ...
    cf_image = sns.heatmap(mc, cmap='Blues')
    figure = cf_image.get_figure()    
    figure.savefig('ej5_' + str(K) + '_folds_Gaussian_'+ approach_name+ "_"+ dataset_name + '.png'); plt.clf()
...
def basico_gaussian_solo_pruebas(dataset_name, dataset_train, dataset_test):
    ...
    for i,L in enumerate(vL):
        for j,G in enumerate(vG):
            for k in range(K):
                modelo=SVC(C=L, kernel ='rbf', gamma=G, verbose=False, decision_function_shape='ovr').fit(tx[k],ty[k])
                z = modelo.predict(vx[k])
                vkappa[k]=cohen_kappa_score(vy[k],z) * 100
            kappa_med=np.mean(vkappa)
            kappa_sintonizacion[i,j] = kappa_med
    ...
    plt.title(f'dataset: {dataset_name}, L_mejor= {L_mellor}, G_mejor: {G_mellor}, kappa={kappa_mellor:.2f}%')
    plt.show()
    #plt.savefig("sintonizacion_SVC_gaussian_"+approach_name+ "_" + dataset_name + ".png"); 
    plt.clf()
    ...
    for k in range(K):
        tx[k]=np.vstack((tx[k],vx[k]))
        ty[k]=np.concatenate((ty[k],vy[k]))
        modelo=SVC(C=L_mellor, kernel ='rbf', gamma=G_mellor, verbose=False, decision_function_shape='ovr').fit(tx[k],ty[k])
        z=modelo.predict(sx[k]); y=sy[k]
        vkappa[k]=100*cohen_kappa_score(y,z)
        v_accuracy[k] = 100*accuracy_score(y,z)
        mc+=confusion_matrix(y,z)
    ...
    cf_image = sns.heatmap(mc, cmap='Blues')
    figure = cf_image.get_figure()    
    #figure.savefig('ej5_' + str(K) + '_folds_Gaussian_'+ approach_name+ "_"+ dataset_name + '.png'); 
    plt.clf()
...
def ej5(dataset_name, dataset_train, dataset_test):
    ...
    # prueba ->
    basico_gaussian_solo_pruebas(dataset_name, dataset_train, dataset_test)
    return
    #####################################################################################3
    ...
    gaussian_kernel_cross_validation(dataset_name, dataset_train, dataset_test, OneVsOneClassifier, 'OVO')

    # PARTE 3: CROSS VALIDATION GAUSSIAN KERNEL OVA (K=4)
    print("EJERCICIO 5_3 (SVM cross_validation, gaussian kernel, OVA, K=4):")
    gaussian_kernel_cross_validation(dataset_name, dataset_train, dataset_test, OneVsRestClassifier, 'OVA')
